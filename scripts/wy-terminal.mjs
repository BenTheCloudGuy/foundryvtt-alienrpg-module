...
async function _autoImportCompendiums() {
  const PACKS_TO_IMPORT = [
    { name: 'wyt-cog-actors', type: 'Actor', folderType: 'Actor' },
  ];

  const imported = game.settings.get('wy-terminal', 'compendiumImported') || {};

  for (const packDef of PACKS_TO_IMPORT) {
    if (imported[packDef.name]) {
      console.log(`WY-Terminal | Compendium '${packDef.name}' already imported (${imported[packDef.name]} docs), skipping.`);
      continue;
    }

    const pack = game.packs.get(`wy-terminal.${packDef.name}`);
    if (!pack) {
      console.warn(`WY-Terminal | Compendium pack 'wy-terminal.${packDef.name}' not found, skipping.`);
      continue;
    }
    ...
    try {
      ...
      if (packFolders && packFolders.size > 0) {
        for (const pf of packFolders) {
          // Check if a folder with this name already exists
          const existing = game.folders.find(f => f.name === pf.name && f.type === packDef.folderType);
          if (existing) {
            folderMap.set(pf._id, existing.id);
            console.log(`WY-Terminal |   Folder '${pf.name}' already exists, reusing.`);
          } else {
            const newFolder = await Folder.create({
              name: pf.name,
              type: packDef.folderType,
              color: pf.color || null,
              sort: pf.sort || 0,
              parent: null,
            });
            folderMap.set(pf._id, newFolder.id);
            console.log(`WY-Terminal |   Created folder '${pf.name}'.`);
          }
        }
      }
      ...
      for (const doc of docs) {
        // Check if this actor already exists in the world (by name + type)
        const exists = game.actors?.find(a => a.name === doc.name && a.type === doc.type);
        if (exists) {
          console.log(`WY-Terminal |   '${doc.name}' already exists, skipping.`);
          continue;
        }

        const data = doc.toObject();
        // Remap folder to world folder
        if (data.folder && folderMap.has(data.folder)) {
          data.folder = folderMap.get(data.folder);
        } else {
          data.folder = null;
        }
      ...
      if (toCreate.length > 0) {
        await Actor.createDocuments(toCreate, { keepId: false });
        console.log(`WY-Terminal |   Imported ${toCreate.length} documents from '${packDef.name}'.`);
      } else {
        console.log(`WY-Terminal |   All documents from '${packDef.name}' already exist.`);
      }

      // Mark as imported
      const updatedImported = { ...imported, [packDef.name]: docs.length };
      await game.settings.set('wy-terminal', 'compendiumImported', updatedImported);
      imported[packDef.name] = docs.length;

      ui.notifications.info(`WY-Terminal: Imported ${toCreate.length} actors from ${packDef.name}.`);
    } catch (err) {
      console.error(`WY-Terminal | Failed to auto-import '${packDef.name}':`, err);
      ui.notifications.error(`WY-Terminal: Failed to import compendium '${packDef.name}'. Check console for details.`);
    }
  }
}
